...
class PostProcessor:
    ...
    def metrics(self) -> dict[str, Any]:
        ...
        if first_open is not None and last_exit is not None:
            delta_us = (last_exit - first_open).total_seconds() if hasattr((last_exit - first_open), "total_seconds") else 0.0
        ...
        daily = (
            positions
            .with_columns(pl.col("exit_time").dt.date().alias("exit_date"))
            .group_by("exit_date")
        ...
        return {
            ...
            "sortino_ratio":       round(sortino_ratio, 4) if math.isfinite(sortino_ratio) else float("inf"),
    ...
    def equity_curve(self) -> pl.DataFrame:
        ...
        return (
            ...
            .sort("exit_time")
            .with_columns(
                (pl.lit(initial) + pl.col("net_pnl").cum_sum()).alias("equity")
            )
    ...
    def _empty_metrics() -> dict[str, Any]:
        return {
            "net_profit": 0.0, "total_trades": 0, "percent_profitable": 0.0,
            "profit_factor": 0.0, "avg_win": 0.0, "avg_loss": 0.0, "median_pnl": 0.0,
            "max_drawdown": 0.0, "max_drawdown_pct": 0.0, "cagr": 0.0, "mar": 0.0,
            "sharpe_ratio": 0.0, "sortino_ratio": 0.0, "calmar_ratio": 0.0,
            "premium_capture_rate": 0.0, "avg_mae": 0.0, "median_mae": 0.0, "worst_mae": 0.0,
        }
